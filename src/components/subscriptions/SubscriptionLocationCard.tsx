...
interface SubscriptionLocationCardProps {
 ...
 * @param props.subscription - The specific subscription detailing fulfillment type and users
 * @returns A card with a interactive map and open in Google Maps button
 */
export function SubscriptionLocationCard({ subscription }: SubscriptionLocationCardProps) {
  const [copied, setCopied] = useState(false);

  const isDelivery = subscription.fulfillmentType?.toLowerCase() === 'delivery';
  const targetUser = isDelivery ? subscription.buyer : subscription.seller;
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              size="sm"
              className="flex-1 bg-white text-xs font-semibold"
              onClick={handleCopy}
            >
              {copied ? (
            ...
            >
              <ExternalLink className="mr-1.5 h-3.5 w-3.5" /> Open Maps
            </Button>
          </div>
        </div>
      </CardContent>
